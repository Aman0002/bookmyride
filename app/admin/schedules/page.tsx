...
import { WEEKDAYS, DEFAULT_SHARED_PRICE, DEFAULT_PRIVATE_PRICE, DEFAULT_CARS } from "@/lib/constants";
import { computeAvailability } from "@/lib/trips";
import { formatDate, formatTimeWindow, formatINR } from "@/lib/utils";
import {
  addSchedule,
  toggleSchedule,
  deleteSchedule,
      ...
      <Card className="p-6">
        ...
        <form action={addSchedule} className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <Input type="time" name="departureTime" required defaultValue="07:00" />
          </div>
          <div>
            <Label>Pickup window end</Label>
            <Input type="time" name="departureEndTime" required defaultValue="08:00" />
          </div>
          <div>
            <Label>Shared price (₹/seat)</Label>
            <Input type="number" name="sharedSeatPrice" defaultValue={DEFAULT_SHARED_PRICE} />
          </div>
          <div>
            <Label>Private price (₹)</Label>
            <Input type="number" name="privatePrice" defaultValue={DEFAULT_PRIVATE_PRICE} />
          </div>
          <div>
            <Label>Cars (pooled)</Label>
            <Input type="number" name="carsCount" defaultValue={DEFAULT_CARS} min={1} />
          </div>
          <div>
            ...
            <Card key={s.id} className="flex items-center justify-between p-4">
              <div>
                <div className="font-medium text-slate-900">
                  ...
                  {formatTimeWindow(s.departureTime, s.departureEndTime)}
                </div>
                <div className="text-sm text-slate-500">
                  {formatINR(s.sharedSeatPrice)}/seat · {formatINR(s.privatePrice)}{" "}
                  private · {s.carsCount} cars ·{" "}
                  {s.daysOfWeek
                    .split(",")
            ...
            <Card key={t.id} className="flex items-center justify-between p-4">
              <div>
                ...
                <div className="text-sm text-slate-500">
                  ...
                  {t.seatsBooked}/{avail.sharedCapacity} shared ·{" "}
                  {t.privateCarsBooked}/{t.carsTotal} cars private
                </div>
              </div>
