...
export async function addSchedule(formData: FormData) {
  ...
  const departureEndTime = String(formData.get("departureEndTime") || "");
  const sharedSeatPrice = Number(formData.get("sharedSeatPrice")) || 600;
  const privatePrice = Number(formData.get("privatePrice")) || 2400;
  const carsCount = Number(formData.get("carsCount")) || 3;
